...
fn read_bound(path: &str, max: usize) -> Result<Vec<u8>> {
    ...
    let mut bytes = Vec::new(); f.take(max as u64 + 1).read_to_end(&mut bytes)?;
    ensure!(bytes.len() <= max, "artifact limit"); Ok(bytes)
}
...
fn main() -> Result<()> {
    ...
    ensure!(a.len() == 6, "prove|verify guest.elf job-commitment-hex input.bin output.bin");
    check_verifier_pin()?;
    ensure!(std::env::var_os("RISC0_DEV_MODE").is_none(), "dev mode forbidden");
    ...
    let job: Hash = hex::decode(&a[3])?.try_into().map_err(|_| anyhow::anyhow!("job digest length"))?;
    ...
    let output = if a[1] == "prove" {
        #[cfg(not(feature="prove"))] { anyhow::bail!("verifier-only binary"); }
        #[cfg(feature="prove")] {
            use risc0_zkvm::{ExecutorEnv,ProverOpts,get_prover_server};
            let mut stdin = job.to_vec(); stdin.extend_from_slice(&(input.len() as u32).to_le_bytes()); stdin.extend_from_slice(&input);
            let env = ExecutorEnv::builder().write_slice(&stdin).segment_limit_po2(16).session_limit(Some(1 << 24)).build()?;
