...
pub const OUTPUT_HEADER_LEN: usize =
    OUTPUT_DOMAIN.len() + JOURNAL_LEN + 32 + 4 + CONTROL_DEPTH * 32 + 4;
...
impl<'a> Batch<'a> {
    ...
    pub fn parse(bytes: &'a [u8]) -> Result<Self> {
        if bytes.len() < INPUT_HEADER_LEN
            ...
            || !bytes.starts_with(INPUT_DOMAIN)
        {
        ...
        Ok(Self {
            ...
            count,
            depth,
            root: bytes[p + 5..p + 37].try_into().unwrap(),
        })
    }
    ...
    pub fn verify(&self, sha256: impl Fn(&[u8]) -> Hash) -> Result<()> {
        for row in self.bytes[INPUT_HEADER_LEN..].chunks_exact(36 + 32 * self.depth as usize) {
            let index = u32::from_le_bytes(row[..4].try_into().unwrap());
            let mut leaf = [0; 33];
            leaf[1..].copy_from_slice(&row[4..36]);
            let mut node = sha256(&leaf);
            for (h, sibling) in row[36..].chunks_exact(32).enumerate() {
                let mut preimage = [0; 65];
                preimage[0] = 1;
                if index & (1 << h) == 0 {
                    preimage[1..33].copy_from_slice(&node);
                    preimage[33..].copy_from_slice(sibling);
                } else {
                    preimage[1..33].copy_from_slice(sibling);
                    preimage[33..].copy_from_slice(&node);
                }
    ...
    pub fn journal(&self, job: Hash, sha256: impl Fn(&[u8]) -> Hash) -> [u8; JOURNAL_LEN] {
        let mut b = [0; JOURNAL_LEN];
        let p = JOURNAL_DOMAIN.len();
...
impl<'a> ProofView<'a> {
    pub fn parse(b: &'a [u8]) -> Result<Self> {
        ...
        let journal = &b[p..p + JOURNAL_LEN];
        p += JOURNAL_LEN;
        if !journal.starts_with(JOURNAL_DOMAIN) {
            return Err("invalid journal domain");
        }
        let n = u32::from_le_bytes(journal[JOURNAL_LEN - 4..].try_into().unwrap());
        if n == 0 || n as usize > MAX_BATCH {
            return Err("journal batch limits");
        }
        let control_id = b[p..p + 32].try_into().unwrap();
        p += 32;
        let control_index = u32::from_le_bytes(b[p..p + 4].try_into().unwrap());
        p += 4;
        if control_index >= 1 << CONTROL_DEPTH {
            return Err("control index bounds");
        }
        let control_path = &b[p..p + CONTROL_DEPTH * 32];
        p += CONTROL_DEPTH * 32;
        let words = u32::from_le_bytes(b[p..p + 4].try_into().unwrap()) as usize;
        p += 4;
        if words == 0 || words > MAX_SEAL_WORDS || b.len() != p + words * 4 {
            return Err("seal bounds or length");
        }
        Ok(Self {
            journal,
            control_id,
